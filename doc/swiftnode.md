...
This mechanism has been integrated into Bitcore-Node-BETXOIN which allows for notification to be broadcast through Insight API in one of two ways:
* WebSocket: 
* API:  
...
By default, the BETXOIN Core daemon will launch using the following constant:

```
static const int DEFAULT_SWIFTNODE_DEPTH = 5;
```

This value can be overridden by passing the following argument to the BETXOIN Core daemon:
