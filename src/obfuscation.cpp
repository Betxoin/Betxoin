...
/* *** BEGIN OBFUSCATION MAGIC - BETXOIN **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat BETXOIN and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 BETXOIN per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    int addLagTime = 0;
    if (!fMasterNode) addLagTime = 1000; //if we're the client, give the server a few extra seconds before resetting.
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeBETXOINAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100BETXOIN+1 ( bit on if present )
    // bit 1 - 10BETXOIN+1
    // bit 2 - 1BETXOIN+1
    // bit 3 - .1BETXOIN+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("betxoin-obfuscation");
