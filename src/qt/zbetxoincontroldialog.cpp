...
#include "ui_zbetxoincontroldialog.h"
...
std::list<std::string> ZBETXOINControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBETXOINControlDialog::listMints;

ZBETXOINControlDialog::ZBETXOINControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBETXOINControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBETXOINControlDialog::~ZBETXOINControlDialog()
{
    delete ui;
}

void ZBETXOINControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBETXOINControlDialog::updateList()
{
...
void ZBETXOINControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBETXOINControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBETXOIN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBETXOINControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBETXOINControlDialog::GetSelectedMints()
{
...
void ZBETXOINControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
