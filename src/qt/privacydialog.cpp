...
#include "zbetxoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBETXOIN ought to be enough for anybody." - Bill Gates, 2017
    ui->zBETXOINpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBETXOINSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBETXOIN->setEnabled(false);
        ui->pushButtonMintzBETXOIN->setToolTip(tr("zBETXOIN is currently disabled due to maintenance."));

        ui->pushButtonSpendzBETXOIN->setEnabled(false);
        ui->pushButtonSpendzBETXOIN->setToolTip(tr("zBETXOIN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBETXOINpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBETXOIN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETXOIN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBETXOIN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBETXOIN in ") +
...
void PrivacyDialog::on_pushButtonSpendzBETXOIN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBETXOIN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBETXOIN
        sendzBETXOIN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBETXOIN
    sendzBETXOIN();
}

void PrivacyDialog::on_pushButtonZBETXOINControl_clicked()
{
    ZBETXOINControlDialog* zBETXOINControl = new ZBETXOINControlDialog(this);
    zBETXOINControl->setModel(walletModel);
    zBETXOINControl->exec();
}

void PrivacyDialog::setZBETXOINControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBETXOINSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBETXOIN()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BETXOIN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBETXOINpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBETXOIN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BETXOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBETXOIN</b>";
    ...
    // use mints from zBETXOIN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBETXOINControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBETXOINControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbetxoin selector in case it was used
    ZBETXOINControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBETXOIN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BETXOIN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBETXOIN Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zBETXOINpayAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBETXOIN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBETXOIN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBETXOIN "));
    ui->labelzBETXOINAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBETXOINSyncStatus->setVisible(fShow);
}
