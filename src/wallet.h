...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 BETXOIN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 1000                          // UTXO's that are valid for staking
};

// Possible states for zBETXOIN send
enum ZerocoinSpendStatus {
    ZBETXOIN_SPEND_OKAY = 0,                            // No error
    ZBETXOIN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBETXOIN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBETXOIN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBETXOIN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBETXOIN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBETXOIN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBETXOIN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBETXOIN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBETXOIN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBETXOIN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBETXOIN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBETXOIN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBETXOIN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBETXOIN_SPENT_USED_ZBETXOIN = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBETXOINAutoBackups(bool fEnabled)
    {
...
public:
    ...
    int Priority() const
    {
        ...
        if (tx->vout[i].nValue < 1 * COIN) return 1000;
